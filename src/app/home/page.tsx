                ...
                {recentOrders.map(order => (
                  <div key={order.id} className="px-5 py-3.5 flex items-center gap-4 table-row-hover transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate" style={{ color: 'hsl(215 25% 12%)' }}>{order.plate} — {order.vehicleType}</p>
                      <p className="text-xs mt-0.5" style={{ color: 'hsl(215 10% 48%)' }}>{order.services.join(', ') || 'No services'}</p>
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold shrink-0 ${
                      order.status === 'Completed' ? 'badge-completed' :
                      order.status === 'In Progress' ? 'badge-in-progress' :
        ...
        <div>
          ...
          <div className="bg-white rounded-xl border" style={{ borderColor: 'hsl(210 18% 89%)' }}>
            ...
            {workers.filter(w => w.status === 'active').length === 0 ? (
              <div className="p-8 text-center">
                <p className="text-sm" style={{ color: 'hsl(215 10% 48%)' }}>No active workers. Add workers in the Workers section.</p>
              </div>
            ) : (
              <div className="divide-y" style={{ borderColor: 'hsl(210 18% 89%)' }}>
                {workers.filter(w => w.status === 'active').map(worker => (
                  <div key={worker.id} className="px-5 py-3.5 flex items-center gap-3 table-row-hover transition-colors">
                    ...
                    <span className="text-xs font-medium hidden sm:block shrink-0" style={{ color: 'hsl(215 10% 48%)' }}>{worker.jobsToday} jobs</span>
