          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                ...
                >
                  {['Order ID', 'Plate', 'Vehicle', 'Services', 'Workers', 'Status', 'Time', 'Actions'].map(
                    (h) => (
                      <th
                        key={h}
                ...
                {filtered.map((wo) => {
                  ...
                  return (
                    <tr key={wo.id} className="table-row-hover transition-colors">
                      ...
                      <td className="px-4 py-3 font-semibold" style={{ color: 'hsl(215 25% 12%)' }}>
                        {wo.plate}
                      </td>
                      ...
                      <td className="px-4 py-3 max-w-[160px]">
                        <p className="text-xs truncate" style={{ color: 'hsl(215 25% 12%)' }}>
                          {wo.services.join(', ') || '—'}
                        </p>
                      </td>
                      ...
                      <td className="px-4 py-3 text-xs" style={{ color: 'hsl(215 10% 48%)' }}>
                        {wo.assignedWorkers.length > 0
                          ? workers
                              .filter((w) => wo.assignedWorkers.includes(w.id))
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${sc.className}`}>
                          {sc.label}
                        </span>
                      </td>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {SERVICE_TYPES.map((svc) => (
                      <label
                        ...
                        style={{
                          borderColor: form.services.includes(svc)
                            ? 'hsl(205 78% 42%)'
                            : 'hsl(210 18% 89%)',
                          backgroundColor: form.services.includes(svc)
                            ? 'hsla(205,78%,42%,0.06)'
                            : 'transparent',
                        }}
                      >
                        ...
                        />
                        <span className="text-xs" style={{ color: 'hsl(215 25% 12%)' }}>
                          {svc}
                        </span>
                      </label>
                    ))}
                  ...
                  <div className="space-y-1.5">
                    {workers.map((w) => (
                      <label
                        ...
                        className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border transition-colors"
                        style={{
                          borderColor: form.assignedWorkers.includes(w.id)
                            ? 'hsl(205 78% 42%)'
                            : 'hsl(210 18% 89%)',
                          backgroundColor: form.assignedWorkers.includes(w.id)
                            ? 'hsla(205,78%,42%,0.06)'
                            : 'transparent',
                        }}
                      >
                        ...
                        <span className="ml-auto text-xs" style={{ color: 'hsl(215 10% 48%)' }}>
                          {w.status}
                        </span>
                      </label>
                    ))}
            ...
            <div className="p-5 space-y-3">
              {[
                ...
                [
                  'Assigned Workers',
                  workers
                    .filter((w) => viewOrder.assignedWorkers.includes(w.id))
                    .map((w) => w.name)
                    .join(', ') || '—',
                ],
                ...
                <div key={label} className="flex justify-between gap-4">
                  <span className="text-xs font-semibold" style={{ color: 'hsl(215 10% 48%)' }}>
                    {label}
                  </span>
                  <span className="text-xs text-right" style={{ color: 'hsl(215 25% 12%)' }}>
                    {value}
                  </span>
                </div>
              ))}
      ...
      {confirmDelete && (
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="font-bold text-lg mb-1" style={{ color: 'hsl(215 25% 12%)' }}>
              Delete Work Order?
            </h3>

            <p className="text-sm mb-5" style={{ color: 'hsl(215 10% 48%)' }}>
