...
import { PlusIcon, PencilSquareIcon, TrashIcon, PlayIcon, CheckIcon, XMarkIcon, TruckIcon, EyeIcon, MagnifyingGlassIcon,  } from '@heroicons/react/24/outline';
import { useAppData, WorkOrder, WorkOrderStatus, VEHICLE_TYPES, SERVICE_TYPES } from '../../../context/AppDataContext';
...
const emptyForm: FormState = {
  plate: '', vehicleType: VEHICLE_TYPES[0], services: [], assignedWorkers: [], notes: '', totalAmount: '',
};

export default function WorkOrdersManager() {
  const { workOrders, workers, addWorkOrder, updateWorkOrder, deleteWorkOrder, startWorkOrder, completeWorkOrder } = useAppData();
  ...
  const openEdit = (wo: WorkOrder) => {
    setForm({
      plate: wo.plate, vehicleType: wo.vehicleType, services: wo.services,
      assignedWorkers: wo.assignedWorkers, notes: wo.notes || '', totalAmount: wo.totalAmount?.toString() || '',
    });
    setEditId(wo.id);
    setShowForm(true);
  };
  ...
  const toggleWorker = (wid: string) => {
    setForm(f => ({
      ...f,
      assignedWorkers: f.assignedWorkers.includes(wid) ? f.assignedWorkers.filter(w => w !== wid) : [...f.assignedWorkers, wid],
    }));
  };

  const filtered = workOrders.filter(wo => {
    const matchSearch = wo.plate.toLowerCase().includes(search.toLowerCase()) ||
        ...
        <div className="flex gap-2 flex-wrap">
          {(['All', 'Pending', 'In Progress', 'Completed', 'Cancelled'] as const).map(s => (
            <button
              ...
              style={filterStatus === s ? { backgroundColor: 'hsl(205 78% 42%)' } : { borderColor: 'hsl(210 18% 89%)', color: 'hsl(215 10% 48%)' }}
            >
              {s}
            </button>
          ))}
      ...
      <div className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: 'hsl(210 18% 89%)' }}>
        {filtered.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-xs mt-1" style={{ color: 'hsl(215 10% 48%)' }}>Click &quot;New Order&quot; to create your first work order</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ backgroundColor: 'hsl(210 20% 98%)', borderBottom: '1px solid hsl(210 18% 89%)' }}>
                  {['Order ID', 'Plate', 'Vehicle', 'Services', 'Workers', 'Status', 'Time', 'Actions'].map(h => (
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {SERVICE_TYPES.map(svc => (
                    <label key={svc} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border transition-colors"
                      style={{ borderColor: form.services.includes(svc) ? 'hsl(205 78% 42%)' : 'hsl(210 18% 89%)', backgroundColor: form.services.includes(svc) ? 'hsla(205,78%,42%,0.06)' : 'transparent' }}>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {workers.map(w => (
                    <label key={w.id} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border transition-colors"
                      style={{ borderColor: form.assignedWorkers.includes(w.id) ? 'hsl(205 78% 42%)' : 'hsl(210 18% 89%)', backgroundColor: form.assignedWorkers.includes(w.id) ? 'hsla(205,78%,42%,0.06)' : 'transparent' }}>
                      <input type="checkbox" checked={form.assignedWorkers.includes(w.id)} onChange={() => toggleWorker(w.id)} className="accent-blue-500" />
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: 'hsla(25,95%,53%,0.15)', color: 'hsl(25 95% 53%)' }}>{w.initials}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { deleteWorkOrder(confirmDelete); setConfirmDelete(null); }} className="flex-1 py-2.5 rounded-lg text-sm font-medium text-white bg-red-500">Delete</button>
            </div>
          </div>
        </div>
      )}
